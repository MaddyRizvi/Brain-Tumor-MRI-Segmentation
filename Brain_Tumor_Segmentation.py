...
def standardize(image):
    ...
    for c in range(image.shape[0]):
        for z in range(image.shape[3]):
            image_slice = image[c,:,:,z].astype(np.float32)
...
def single_class_dice_coefficient(y_true, y_pred, axis=(0, 1, 2), 
                                  epsilon=0.00001):
    dice_numerator = 2.0 * K.sum(y_true * y_pred, axis=axis) + epsilon
...
def dice_coefficient(y_true, y_pred, axis=(1, 2, 3), epsilon=0.00001):
    ...
    dice_denominator = K.sum(y_pred, axis=axis) + K.sum(y_true, axis=axis) + epsilon
    per_class_dice = dice_numerator / dice_denominator
    dice_coefficient = K.mean(per_class_dice)
    return dice_coefficient
...
def soft_dice_loss(y_true, y_pred, axis=(1, 2, 3), epsilon=0.00001):
    dice_numerator = 2.0 * K.sum(y_pred * y_true, axis=axis) + epsilon
    dice_denominator = K.sum(K.square(y_pred), axis=axis) + K.sum(K.square(y_true), axis=axis) + epsilon
    per_class_dice = dice_numerator / dice_denominator
    dice_loss = 1.0 - K.mean(per_class_dice)
    return dice_loss
...
def compute_class_sens_spec(pred, label, class_num):
    class_pred = pred[class_num]
    class_label = label[class_num]
    class_pred = np.asarray(class_pred).astype(np.int32)
    class_label = np.asarray(class_label).astype(np.int32)
